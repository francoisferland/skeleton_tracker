...
public:
      ...
      it_(nh_)
  {
    ...
    if (depthStream_.create(devDevice_, openni::SENSOR_DEPTH) == openni::STATUS_OK)
    {
      ...
      depthStream_.setMirroringEnabled(false);// both parameters (depth and color stream) settled as "true", it allows a superposition of video and tracking image not mirrored.
    }
    else
    {
